...
class ElaDataTable(ElaTableView):
    ...
    def setTableData(
        ...
        *data* 支持两种格式：
        ...
        :param center_columns: 需要居中对齐的列索引集合。设为 ``None`` 时不应用特殊对齐。
        ...
        for row_idx, row_data in enumerate(rows):
            ...
            if center_columns:
                for col_idx in center_columns:
                    items[col_idx].setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            ...
            try:
                ...
                for row_idx, row_data in enumerate(rows_data):
                    for col_idx, cell_data in enumerate(row_data):
                        self._model.setItem(
                            row_idx, col_idx, QStandardItem(str(cell_data))
                        )
            finally:
                self._model.blockSignals(False)
            self._model.layoutChanged.emit()
